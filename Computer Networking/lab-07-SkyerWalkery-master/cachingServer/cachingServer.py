...
class CachingServer(TCPServer):
    ...
    def touchItem(self, path: str) \
        ...
        # Cannot found in cache or expired
        # Fetch from main server
        # Note: use stream forwarding to accelerate the process
        else:
            ...
            read_len = reponse.readinto(body_parts)
            while read_len:
...
class CachingServerHttpHandler(BaseHTTPRequestHandler):
    ...
    @trace
    def sendHeaders(self, headers_vals: List[Tuple[str, str]]) -> None:
        ...
        for header, val in headers_vals:
            self.send_header(header, val)
        self.end_headers()

    def sendBody(self, body) -> None:
    ...
    def do_HEAD_GET(self, is_get: bool) -> None:
        ret_gen = self.server.touchItem(self.path)
